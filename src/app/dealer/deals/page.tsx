...
const activeDealsData = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1614200187524-dc4b892acf16?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80",
  ...
  },
  {
    ...
    buyer: "Adam Smith",
    image:
      "https://images.unsplash.com/photo-1592198084033-aade902d1aae?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80",
    progress: 2,
    price: "510,000$",
  },
  {
      ...
      <section>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {activeDealsData.map((deal) => (
            <div
              key={deal.id}
              className="bg-[#24252A] rounded-xl p-5 flex flex-col gap-6 border border-white/5 shadow-md"
            >
              {/* Header */}
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-1">
                  <div
                    className={`text-xs font-semibold px-2.5 py-1 rounded w-max mb-2 ${
                      deal.status === "Active negotiation"
                        ? "bg-[#002C14] text-[#00A34A]"
                        : deal.status === "Offer Received"
                          ? "bg-[#002540] text-[#007AE6]"
              ...
              <div className="px-2 mt-2">
                <div className="relative flex justify-between mt-4 z-0 pb-8">
                  ...
                  <div
                    className="absolute top-2.75 left-2.75 h-0.75 bg-[#d3a436] -z-10 transition-all duration-300 rounded-full"
                    style={{
                      width: `calc(${((deal.progress - 1) / (steps.length - 1)) * 100}% - ${((deal.progress - 1) / (steps.length - 1)) * 22}px)`,
                    }}
                  ></div>
                  ...
                  {steps.map((step, index) => {
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center relative z-10 w-12">
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center shadow-sm ${isCompleted ? "bg-[#d3a436]" : "bg-[#4c4d52]"}`}
                        >
                          <Check
                            className={`w-4 h-4 ${isCompleted ? "text-[#24252A]" : "text-[#24252A]/50"}`}
                            strokeWidth={3.5}
                          />
                        </div>
                        <span className="text-[11px] text-gray-400 mt-3 absolute top-5.5 whitespace-nowrap font-medium tracking-wide">
                          {step}
                        </span>
                      </div>
                    );
              ...
              <div className="flex justify-between items-center mt-2 border-t border-white/5 pt-5">
                ...
                <Link
                  ...
                  className="flex items-center gap-2 bg-[#d3a436] hover:bg-[#c29631] text-[#24252A] px-5 py-2.5 rounded-md text-sm font-bold transition-colors"
                >
      ...
      <section className="mb-10">
        ...
        <div className="flex flex-col gap-4">
          {closedDealsData.map((deal) => (
            <div
              key={deal.id}
              className="bg-[#24252A] rounded-xl p-5 flex flex-col md:flex-row justify-between md:items-center gap-4 md:gap-0 border border-white/5 shadow-md"
            >
              <div className="flex flex-col">
                <h3 className="text-lg font-semibold tracking-wide">
                  {deal.carName} - {deal.buyer}
                </h3>
